...
function Logo({ size = 'md', showText = true, className = '' }) {
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      ...
      {showText && (
        <div>
          <div className={`font-black tracking-tight ${textClass} bg-gradient-to-r from-teal-200 via-cyan-200 to-emerald-200 bg-clip-text text-transparent`}>
            SwasthaLink
          </div>
          <p className={`${subtitleClass} uppercase tracking-[0.2em] text-teal-200/70 font-semibold`}>
            Ethereal Clinic
          </p>
        </div>
      )}
